MAPBOX_TOKEN =
  "pk.eyJ1IjoibGVvbmFyZGh1eW5oIiwiYSI6ImNscDNxdzNmZzB6dG0ya3M1MGt2MTVreHEifQ.WiaFF1ZoklZy7vMDLcPJ5g";
...
function addAdvertisementSource(map) {
  map.loadImage("assets/blue_elip.png", (error, image) => {
  ...
  map.on("click", "quang-cao", function (e) {
    map.flyTo({
      center: [
        e.features[0].geometry.coordinates[0] + 0.005,
...
function addReportSource(map) {
  map.loadImage("assets/red_elip.png", (error, image) => {
    if (error) throw error;
  ...
  map.on("click", "bao-cao", function (e) {
    map.flyTo({
      center: [
        e.features[0].geometry.coordinates[0] + 0.02,
        e.features[0].geometry.coordinates[1],
      ],
      essential: true, // this animation is considered essential with respect to prefers-reduced-motion
      zoom: 14,
...
mapboxScript.onload = function () {
  ...
  map.on("style.load", function (e) {
    map.on("click", function (e) {
      var coor = e.lngLat;
      new mapboxgl.Popup().setLngLat(coor).setHTML(coor).addTo(map);
    });
  });
};
